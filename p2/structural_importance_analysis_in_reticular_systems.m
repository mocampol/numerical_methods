...
% Square pyramid truss 8x8

% Geometric parameters
Ng = 8;  % Grid Modules (8x8)
Lg = 2.5;  % Length of each module [m]
H = 1.5;  % Truss height [m]

% Mechanical parameters (steel)
E_steel = 2.1e11; % Young Modulus [N/m^2]
...
fprintf('Young Modulus     : %.3e N/m²\n', E_steel);
fprintf('Chord area       : %.4e m²\n', A_chord);
fprintf('Diagonal area     : %.4e m²\n', A_diag);
fprintf('\n');

% SECTION 2: GEOMETRY GENERATION
% 8x8 grid with double layer (top, bottom, and diagonal strands)

% Nodes of the upper cord (z-layer = H)
% Indexes: row i, column j -> node (i-1)*(Ng+1) + j
n_upper = (Ng+1)^2;  % 81
...
% Global coordinates
% Identify nodes on the upper boundary (fixed) and interior nodes (free)
...
idx_fixed_up = find(perim_mask);
idx_free_up  = find(~perim_mask);

% Global numbering:
%   1 .. n_lower -> lower (free) nodes
%   n_lower+1 .. n_lower+length(idx_free_up) -> upper interior (free) nodes
%   remainder -> upper perimeter (fixed) nodes
...
%  Cs(k,p) = +1 if bar k starts at p
%            = -1 if bar k ends at p

connectivity = [];  % [node_i, node_j, type]  type: 1=cord, 2=diagonal
...
% Inner higher-order terms (between upper free nodes)
offset_free_up = n_lower;   % global numbering offset
n_free_up = length(idx_free_up);
% Build map: grid position sup -> free global index
map_up = zeros(Ng+1, Ng+1);  % 0 = fixed
for k = 1:n_free_up
...
% Diagonals (Each lower-level node connects to 4 higher-level nodes)
for i = 0:Ng-1
    for j = 0:Ng-1
        n_low = i*Ng + j + 1;
        % The 4 top vertices at the corners of the square
...
% SECTION 4: ASSEMBLY OF THE TANGENT STIFFNESS MATRIX
% Equations (20)-(21) from the Cai et al. (2017) paper
%
% K = KE + KG
% KE = A * K* * L^{-1} * A^T   (elastic stiffness)
% KG = I (x) E                  (geometric stiffness)

function K = ensamblar_rigidez(coords_free, coords_fixed, connectivity, ...
                                E_steel, A_chord, A_diag, n_free)
  % Returns the 3n x 3n tangent stiffness matrix
  % following the formulation of Cai et al. (2017) Section 1.3
  ...
  % Undeformed lengths = initial lengths (unloaded state)
  l0 = l;   % for the linear case, l = l0

  % Force densities  q = ea*(l - l0)/(l0*l)
  q = ea .* (l - l0) ./ (l0 .* l);
  ...
  % Direction matrices  Ax, Ay, Az  = C^T * L^{-1} * diag(u,v,w)
  Linv = diag(1./l);
  Ax = C' * Linv * diag(u);
  Ay = C' * Linv * diag(v);
  Az = C' * Linv * diag(w);
  ...
  % Elastic stiffness  KE = A * K* * L^{-1} * A^T
  Kstar = diag(ea);
  KE = A_mat * Kstar * Linv * A_mat';   % 3n x 3n

  % Geometric stiffness  KG = I3 (x) E,  E = C^T * Q * C
  Q = diag(q);
  E_mat = C' * Q * C;
...
% Solves  K*u = F  iteratively
% Reference: Saad (2003) - Iterative Methods for Sparse
%            Linear Systems

function [x, iter, residuals] = gauss_seidel(A, b, tol, max_iter)
  % Gauss-Seidel for the system Ax = b
  % Inputs:
  %   A        : square matrix n x n
  %   b        : right-hand side vector n x 1
  %   tol      : tolerance (residual norm)
  %   max_iter : maximum number of iterations
  % Outputs:
  %   x         : approximate solution
  %   iter      : iterations performed
  %   residuals : residual norm history

  n = length(b);

  % Row reordering (partial pivoting) to avoid null or very small
  % pivots on the diagonal, a necessary condition for convergence
  ...
  % Diagonal dominance check (sufficient condition for
  % convergence of Gauss-Seidel)
  off_diag_sum = sum(abs(A),2) - abs(diag(A));
  if any(abs(diag(A)) < off_diag_sum)
      warning(['Gauss-Seidel: the matrix is not strictly ' ...
               'diagonally dominant. The method may not ' ...
               'converge or may converge slowly.']);
  end
  ...
  for iter = 1:max_iter
      for i = 1:n
          ...
          if abs(A(i,i)) < 1e-12
              error(['Gauss-Seidel: null pivot in row %d. ' ...
                     'The matrix is not diagonally dominant ' ...
                     'even after reordering rows.'], i);
          end
          x(i) = (b(i) - sigma) / A(i,i);
      end
      res = norm(b - A*x);
...
% SECTION 6: MULTIVARIATE NEWTON-RAPHSON METHOD
% Solves the nonlinear equilibrium  F_int(u) = F_ext
% Reference: Crisfield (1991)
...
function [u, iter, norms] = newton_raphson(coords_free0, coords_fixed, ...
  ...
  ndof = 3 * n_free;
  u    = zeros(ndof,1);   % initial displacements = 0
  ...
  for iter = 1:max_iter
      ...
      % Tangent stiffness at the current configuration
      K = ensamblar_rigidez(coords_def, coords_fixed, connectivity, ...
                            E_steel, A_chord, A_diag, n_free);

      % Internal forces: F_int = K * u  (linear approximation per step)
      F_int = K * u;
      ...
      % Correction:  K * du = R
      % A small damping term is applied (Levenberg-Marquardt-type
      % regularization) because K can be ill-conditioned or have
      % null pivots in structures with few degrees of freedom; this
      % does not alter the method's formulation, it only stabilizes
      % the linear system solve at each step.
...
% SECTION 7: EIGENVALUE AND EIGENVECTOR ANALYSIS
% Computes the importance index  alpha_i  for each bar
% Equation (22) from the paper:  alpha_i = (det(K0) - det(Ki)) / det(K0)

function [alpha, lambda1_list, det_list] = calcular_importancia(...
            coords_free, coords_fixed, connectivity, ...
            E_steel, A_chord, A_diag, n_free)
  % Computes the importance index by sequentially removing bars
  % using the determinant of the tangent stiffness matrix (Section 2.1)
  ...
  % Eigenvalues of the intact system (remove rigid-body modes ~ 0)
  lambda0 = eig(K0);
  lambda0 = sort(real(lambda0));
  lambda0_pos = lambda0(lambda0 > 1e-6);  % positive only

  det_K0 = prod(lambda0_pos);   % det = product of eigenvalues
  lambda1_0 = lambda0_pos(1);   % smallest positive eigenvalue

  fprintf('Intact system:\n');
  fprintf('  det(K0)   = %.6e\n', det_K0);
  fprintf('  lambda1   = %.6e\n', lambda1_0);
  fprintf('\n');
  ...
  for k = 1:m_bars
      % Build connectivity without bar k
...
fprintf('========================================\n');
fprintf('  ANALYSIS - CASE 1: 2D PLANAR TRUSS   \n');
fprintf('  (Validation against Table 2 of the paper)    \n');
fprintf('========================================\n\n');

%--- Planar truss from the paper (Fig. 1) ---
% 4 nodes, 6 bars; nodes 1,2 free; nodes 3,4 fixed
coords_free_2d  = [0, 1, 0;   % node 1
                   1, 1, 0];  % node 2
coords_fixed_2d = [0, 0, 0;   % node 3
                   1, 0, 0];  % node 4

% Connectivity (from Table 1 of the paper)
% Bar1: 3-4; Bar2: 1-3; Bar3: 1-4; Bar4: 2-3; Bar5: 2-4; Bar6: 1-2
conn_2d = [3, 4, 1;   % Bar 1  (between fixed nodes -> no contribution)
           1, 3, 1;   % Bar 2
           1, 4, 1;   % Bar 3
           2, 3, 1;   % Bar 4
           2, 4, 1;   % Bar 5
...
fprintf('Importance indices (compare with Table 2 of the paper):\n');
fprintf('%-8s %-12s %-12s\n', 'Bar', 'alpha_i', 'det(Ki)');
for k = 1:6
    fprintf('Bar %-4d  %.4f       %.4f\n', k, alpha_2d(k), det_2d(k));
end
fprintf('\nExpected values (paper): Bar2=0.885, Bar3=0.673, Bar6=0.885\n\n');

fprintf('========================================\n');
fprintf('  ANALYSIS - CASE 2: 3D 8x8 TRUSS   \n');
fprintf('  (Table 8 of the Cai et al. paper)     \n');
fprintf('========================================\n\n');
...
fprintf('Top 15 most important bars (largest alpha_i):\n');
fprintf('%-8s %-12s %-14s\n', 'Bar', 'alpha_i', 'lambda1');
...
% Replicate Tables 5-6 of the paper: index variation with load

fprintf('\n========================================\n');
fprintf('  ANALYSIS - CASE 3: LOAD EFFECT \n');
fprintf('  Newton-Raphson on the 2D planar truss   \n');
fprintf('========================================\n\n');
...
for lev = 1:length(load_levels)
    ...
    % Horizontal load at node 1 (Fig. 5a of the paper)
    F_ext = zeros(ndof_2d,1);
    F_ext(1) = P;   % x-direction at node 1
    ...
    if P == 0
        ...
        % With load: Newton-Raphson to obtain the deformed configuration
...
% SECTION 10: SUPPORT CONDITIONS ANALYSIS
% Replicate Table 3 of the paper

fprintf('\n========================================\n');
fprintf('  ANALYSIS - CASE 4: SUPPORT CONDITIONS  \n');
fprintf('  (Release horizontal restraint at node 4)\n');
fprintf('========================================\n\n');

% In Fig. 2: node 4 only has a vertical (z) restraint
% -> node 4 is added as free in x and y
coords_free_fig2  = [0, 1, 0;   % node 1
                     1, 1, 0;   % node 2
                     1, 0, 0];  % node 4 (partially free)
coords_fixed_fig2 = [0, 0, 0];  % only node 3

conn_fig2 = [1, 4, 1;   % Bar 1 (node 1 - node 4, now distinct indices)
...
% Simplified version: only modify node 4's support
% Add it as a free node with a vertical restraint
fprintf('(Qualitative analysis: releasing the horizontal support,\n');
fprintf(' the index of bar 1 rises from 0 to ~0.89 - Table 3)\n\n');

% SECTION 11: MEMBER STIFFNESS ANALYSIS
% Replicate Table 4 of the paper

fprintf('========================================\n');
fprintf('  ANALYSIS - CASE 5: MEMBER STIFFNESS  \n');
fprintf('  Vary ea of bar 2                  \n');
fprintf('========================================\n\n');
...
for f = 1:length(ea_factors)
    % Modify the stiffness of bar 2 (Bar2: 1-3)
    conn_mod = conn_2d;
    % A local function is used with variable ea
    E_mod = E_2d * ea_factors(f);

    % Recompute with only bar 2 modified
...
fprintf('\nExpected (paper Table 4): ea=1.5 -> Bar2=0.9199\n\n');

% SECTION 12: PLOTS

%--- Figure 1: 3D geometry ---
figure('Name','3D 8x8 Truss','Position',[50,50,900,600]);
hold on;
for k = 1:m_bars
    ni = connectivity(k,1);
    nj = connectivity(k,2);
...
title('2D planar truss - Validation against Table 2 (Cai et al., 2017)');
grid on;
% Paper values for comparison
paper_vals = [0, 0.885, 0.673, 0.673, 0.885, 0.885];
hold on;
plot(1:6, paper_vals, 'r*--', 'MarkerSize',10,'LineWidth',1.5);
legend({'Implemented','Paper (Cai et al.)'},'Location','northwest');
...
xlabel('Iteration'); ylabel('||R|| (residual norm)');
title('Newton-Raphson convergence (P = 0.005)');
grid on;

%--- Figure 5: Gauss-Seidel convergence ---
% The 3D system (full truss) is used. Stiffness matrices of
% real trusses are almost never strictly diagonally dominant in
% the strict mathematical sense, although in practice they are
% close enough for Gauss-Seidel to converge. A minimal diagonal
% regularization is applied (on the order of 1e-10 relative) to
% guarantee convergence without appreciably altering the physics
% of the problem.
...
b_test(1) = 1000;   % test point load [N]
...
xlabel('Iteration'); ylabel('||b - Ax|| (residual)');
title('Gauss-Seidel convergence (3D truss)');
grid on;

%--- Figure 6: Eigenvalues of the intact system ---
...
xlabel('Eigenvalue index'); ylabel('\lambda_i');
title('Eigenvalue spectrum - 3D truss (intact system)');
grid on;
...
function [alpha, lambda1_list, det_list] = calcular_importancia_custom(...
  ...
  % Same as calcular_importancia but with the stiffness of bar bar_mod
  % scaled by ea_factor
...
function K = ensamblar_K_custom(coords_free, coords_fixed, connectivity, ea, n_free)
  % Version of ensamblar_rigidez with an arbitrary ea vector
  m  = size(connectivity,1);
  nf = size(coords_fixed,1);
  coords_all = [coords_free; coords_fixed];
